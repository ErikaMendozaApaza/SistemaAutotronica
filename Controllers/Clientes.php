...
/*         if(empty($_SESSION['activo'])){
    ...
    public function listar(){
        ...
        for($i=0;$i<count($data);$i++){
            ...
            <button class="btn btn-success" type="button" onclick="btnReingresarCliente('.$data[$i]['id'].');">Reingresar</button>
            </div>';
    ...
    function registrar(){
        ...
        $id=$_POST['id'];
        if(empty($ci) || empty($nombres) || empty($apellidopaterno) || empty($apellidomaterno) || empty($direccion) || empty($correo) || empty($telefono)){
            $msg="Todos los campos son obligatorios";
        }else{
            if($id==""){
                    $data=$this->model->registrarCliente($ci,$nombres,$apellidopaterno,$apellidomaterno,$direccion,$correo,$telefono);
                    if($data=="ok"){
                        $msg="si";
                    }else if($data=="existe"){
                        $msg="El carnet de identidad de Cliente ya existe";
                    }else{
                        $msg="Error al registrar el cliente";
                    }
            }else{
                $data=$this->model->modificarCliente($ci,$nombres,$apellidopaterno,$apellidomaterno,$direccion,$correo,$telefono,$id);
                if($data=="modificado"){
                    $msg="modificado";
                }else{
